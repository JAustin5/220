...
def play_graphics(secret_word):
    ...
    while (tries > 0) and not won(make_hidden_secret(secret_word, user_guesses)):
        while val_guess:
            ...
            letters_guessed = str(user_guesses)
            if not already_guessed(user_input, user_guesses):
                word_bank = Text(Point(317.5, 120), user_guesses.append(user_input))
                word_bank.setText(letters_guessed)
                word_bank.getText()
                word_bank.draw(win)
                val_guess = False
    ...
    if won(make_hidden_secret(secret_word, user_guesses)):
        win_game = Text(Point(317.5, 90), 'Winner!')
        win_game.draw(win)
    else:
        lost_game = Text(Point(317.5, 90), 'Sorry, you did not guess the word. The secret word was ' + secret_word)
        lost_game.draw(win)
